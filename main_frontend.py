...
import Trilio_Blockchain_Mian as tbm  # importing Trilio_Blockchain_Main.py
...
with st.sidebar:
    ...
    st.write(
        "This is a crypto wallet simulation project made with streamlit. Refresh after every actions to see the results")

    st.title("Total number of wallets in the network: " + str((total_wallets)))
    st.title("Block height: " + str((len(all_transactions_amount))))

    st.write(
        "The block height is the same as the length of the block, this means it is the number of transactions that occured")
